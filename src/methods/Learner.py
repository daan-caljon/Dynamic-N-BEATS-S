...
from src.methods.utils.metrics import MAPE, RMSSE, SMAPE, RMSSE_m
from src.methods.utils.NashMTL import WeightMethods, auxinash_hyperstep
...
class StableNBeatsLearner:
    def __init__(self, device, forecast_length, configNBeats):
        gc.collect()
        ...
        if self.configNBeats["balance_type"] == "uw":
            ...
            self.optim = torch.optim.Adam(
                list(self.model.parameters()) + [self.loss_scale],
    ...
    def ts_padding(self, ts_train_data, ts_eval_data):
        ...
        ts_train_pad = [
            x
            if x.size >= length_train
            else np.pad(
                x, (int(length_train - x.size), 0), "constant", constant_values=0
            )
            for x in ts_train_data
        ]
        ts_eval_pad = [
            x
            if x.size >= length_eval
            else np.pad(
                x, (int(length_eval - x.size), 0), "constant", constant_values=0
            )
            for x in ts_eval_data
        ]
    ...
    def make_batch(self, batch_data, shuffle_origin=True):
        ...
        target_arr = np.empty(
            shape=(len(batch_data), self.shifts + 1, self.forecast_length)
        )
                ...
                for shift in range(self.shifts + 1):
                    if shift == 0:
                        ...
                        x_arr[j, shift, :] = i[
                            ...
                            - self.configNBeats["backcast_length_multiplier"]
                            * self.forecast_length
                            - shift : -self.forecast_length - shift
                        ]
                        target_arr[j, shift, :] = i[
                            -self.forecast_length - shift : -shift
                        ]
            else:
                for shift in range(self.shifts + 1):
                    x_arr[j, shift, :] = i[
                        ...
                        * self.forecast_length
                        - offset
                        - shift : -self.forecast_length - offset - shift
                    ]
                    target_arr[j, shift, :] = i[
                        -self.forecast_length - offset - shift : -offset - shift
                    ]

        return x_arr, target_arr

    def create_example_plots(
        self, output, target, actuals_train, final_evaluation=False
    ):
        ...
        fig.add_trace(
            go.Scatter(
                x=x_axis.numpy(),
                y=random_sample_forecasts.numpy(),
                mode="lines+markers",
                name="forecasts",
            )
        )
    ...
    def evaluate(self, x_arr, target_arr, epoch=None, need_grad=True, early_stop=False):
        losses = dict()
        ...
        if need_grad:
            ...
            forecast_arr = self.model(x_arr)

            losses_forecast_shifts = 0.0
            for shift in range(self.shifts + 1):
                losses_forecast_shifts += self.loss(
                    forecast_arr[:, shift, :],
                    target_arr[:, shift, :],
                    x_arr[:, shift, :],
                )
            ...
            if self.shifts > 0:
                ...
                forecast_base_arr = torch.zeros(
                    (
                        forecast_arr.shape[0],
                        sum(
                            range(
                                self.forecast_length - self.shifts, self.forecast_length
                            )
                        ),
                    ),
                    dtype=torch.float,
                ).to(self.device)
                forecast_shift_arr = torch.zeros(
                    (
                        forecast_arr.shape[0],
                        sum(
                            range(
                ...
                for shift in range(1, self.shifts + 1):
                    for horizon_m1 in range(self.forecast_length - shift):
                        ...
                        forecast_shift_arr[:, col] = forecast_arr[
                            :, shift, horizon_m1 + shift
                        ]
                        col = col + 1
                losses["forecast_stability"] = self.loss(
                    forecast_shift_arr, forecast_base_arr, x_arr[:, 0, :]
                )
            else:
            ...
            with torch.no_grad():
                ...
                if self.shifts > 0:
                    forecast_base_arr = torch.zeros(
                        (
                            ...
                            sum(
                                range(
                                    self.forecast_length - self.shifts,
                                    self.forecast_length,
                                )
                            ),
                        ),
                    ...
                    losses["forecast_stability"] = self.loss(
                        forecast_base_arr, forecast_shift_arr, x_arr[:, 0, :]
                    )
                else:
                    losses["forecast_stability"] = torch.zeros(1)
                ...
                if not self.disable_plot:
                    if early_stop:
                        ...
                        self.create_example_plots(
                            forecast_arr[0, 0, :], target_arr[0, 0, :], x_arr[0, 0, :]
                        )
                    else:
    ...
    def train_net(
        ...
        for epoch in range(1, self.configNBeats["epochs"] + 1):
            ...
            for k in range(num_batches):
                ...
                batch = np.array(
                    ts_train_pad[
                        k * self.configNBeats["batch_size"] : (k + 1)
                        * self.configNBeats["batch_size"]
                    ]
                )
                x_arr, target_arr = self.make_batch(batch, shuffle_origin=True)

                self.optim.zero_grad()  # Set gradients to zero
                losses_batch = self.evaluate(
                    x_arr, target_arr, epoch, need_grad=True, early_stop=False
                )

                if (
                    self.configNBeats["balance_type"] == "gcossim"
                    or self.configNBeats["balance_type"] == "weighted gcossim"
                ):
                    ...
                    main_grad_flat = torch.cat(
                        tuple(
                            g.reshape(
                                -1,
                            )
                            for i, g in enumerate(my_temp_list)
                        ),
                        axis=0,
                    )
                    ...
                    cosine = torch.clamp(
                        nn.CosineSimilarity(dim=0)(main_grad_flat, aux_grad_flat), -1, 1
                    )
                    print(cosine, "cosine")
                        ...
                        for g, ga in zip(grad, aux_grad):
                            if g is not None:
                                if self.configNBeats["balance_type"] == "gcossim":
                                    ...
                                    self.configNBeats["balance_type"]
                                    == "weighted gcossim"
                                ):
                ...
                # uncertainty waiting adapted from https://github.com/median-research-group/LibMTL/blob/main/LibMTL/weighting/UW.py
                    ...
                    loss_combined = (
                        losses_batch["forecast_accuracy"] * weights_cloned[0]
                        + losses_batch["forecast_stability"] * weights_cloned[1]
                    )
                    self.optim.zero_grad()
                    loss_combined.backward()
                    self.optim.step()
                    self.mylambda = weights[1] / (weights[0] + weights[1])
                elif self.configNBeats["balance_type"] == "nashmtl":
                    print("yes")
                    print(epoch, k)
                    if epoch == 1 and k == 0:
                        print("in if")
                        self.weight_method = WeightMethods(
                            ...
                            n_tasks=2,
                            device=self.device,
                            update_weights_every=self.configNBeats[
                                "update_weights_every"
                            ],
                            optim_niter=self.configNBeats["optim_niter"],
                            max_norm=self.configNBeats["max_norm"],
                            scale_alpha=self.configNBeats["scale_alpha"],
                        )
                    print("nashmtl")
                    self.optim.zero_grad()
                    losses = torch.stack(
                        [
                            losses_batch["forecast_accuracy"],
                            losses_batch["forecast_stability"],
                        ]
                    )

                    # loss,extra_outputs = self.weight_method.get_weighted_loss(losses=losses,shared_parameters=list(self.model.parameters()))
                    loss, extra_outputs = self.weight_method.backward(
                        losses=losses, shared_parameters=list(self.model.parameters())
                    )
                    loss_combined = loss
                    # calculate my_lambda
                    self.mylambda = extra_outputs["weights"][1] / (
                        extra_outputs["weights"][0] + extra_outputs["weights"][1]
                    )
                    print(extra_outputs["weights"])
                    self.optim.step()
                elif self.configNBeats["balance_type"] == "auxinash":
                    # For the hypersteps we follow the paper where they use a training batch as the validation loss and another training batch as training loss
                    # We take the the next two batches, which is why we skip training for those

                    if epoch == 1 and k == 0:
                        p = [
                            self.configNBeats["preference_accuracy"],
                            1 - self.configNBeats["preference_accuracy"],
                        ]
                        self.weight_method = WeightMethods(
                            method="auxinash",
                            n_tasks=2,
                        ...
                        aux_params_alpha = torch.tensor(
                            self.weight_method.method.prvs_alpha,
                            dtype=torch.float32,
                            device=self.device,
                            requires_grad=True,
                        )
                        aux_params_p = torch.tensor(
                            self.weight_method.method.p,
                            device=self.device,
                            requires_grad=True,
                            dtype=torch.float32,
                        )
                        meta_opt = torch.optim.Adam(
                            [aux_params_p],
                            lr=self.configNBeats["preference_lr"],
                        )
                    ...
                    hyperstep += 1
                    # In the paper they use a training batch as the validation loss and another training batch as training loss
                    if (
                        hyperstep % self.configNBeats["hyperstep"]
                    ) == 0 and self.configNBeats["auto_p"]:
                        # get losses for this batch and for the next batch
                        print("hyperstep p:", aux_params_p)
                        meta_opt.zero_grad()
                        # randomly shuffle the training data and take the first 2 batches:
                        ts_hyperstep_pad = ts_train_pad.copy()
                        np.random.shuffle(ts_hyperstep_pad)
                        batch_train = np.array(
                            ts_hyperstep_pad[
                                (0) * self.configNBeats["batch_size"] : (1)
                                * self.configNBeats["batch_size"]
                            ]
                        )
                        x_arr_train, target_arr_train = self.make_batch(
                            batch_train, shuffle_origin=True
                        )
                        batch_val = np.array(
                            ts_train_pad[
                                (1) * self.configNBeats["batch_size"] : (2)
                                * self.configNBeats["batch_size"]
                            ]
                        )
                        x_arr_val, target_arr_val = self.make_batch(
                            batch_val, shuffle_origin=True
                        )
                        # calculate losses for both batches
                        losses_train = self.evaluate(
                            x_arr_train,
                            target_arr_train,
                            epoch,
                            need_grad=True,
                        ...
                        losses_train = [
                            losses_train["forecast_accuracy"],
                            losses_train["forecast_stability"],
                        ]  # create list
                        losses_val = self.evaluate(
                            x_arr_val,
                            target_arr_val,
                            epoch,
                            need_grad=True,
                        ...
                        val_loss_accuracy = losses_val["forecast_accuracy"]
                        auxinash_hyperstep(
                            model=self.model,
                        ...
                        meta_opt.step()
                        self.weight_method.method.set_p(
                            aux_params_p.detach().cpu().numpy()
                        )
                        with torch.no_grad():
                            aux_params_p.data = torch.from_numpy(
                                self.weight_method.method.p
                            ).to(self.device)
                        print("hyperstep p:", aux_params_p)
                        #This saves the hyperstep p value to wandb
                        #Name is not ideal
                        wandb.log({"hyperstep_p": aux_params_p[0]})

                elif self.configNBeats["balance_type"] == "gradnorm":
                    # torch.autograd.set_detect_anomaly(True)
                    print(torch.cuda.memory_reserved(device="cuda:0"))
                    losses_batch_tensor = torch.tensor(
                        [
                            losses_batch["forecast_accuracy"],
                            losses_batch["forecast_stability"],
                        ],
                    ...
                    if epoch == 1 and k == 0:
                        balance_weights = torch.ones(
                            2, device=self.device
                        )  # weights used to balance tasks: weight1*accuracy + weight2*instability
                        balance_weights[0] = 1.9
                        balance_weights[1] = (
                            0.1  # start weights
                        )
                        balance_weights = torch.nn.Parameter(
                            balance_weights
                        )  # So gradient can be calculated
                        T = 2  # 2 tasks
                        self.optim_grad = torch.optim.Adam(
                            [balance_weights],
                            lr=self.configNBeats["learning_rate_gradnorm"],
                        )  # create second optimizer
                        l0 = losses_batch_tensor.detach()  # loss in first iteration
                        # get right layer:
                        myblock = self.model.stacks[self.configNBeats["n_stacks"] - 1][
                            self.configNBeats["nb_blocks_per_stack"] - 1
                        ]  # last blcok
                        lastlayer = (
                            myblock.fc4
                        )  # (last shared layer of last block of laatste stack
                    balance_weights_cloned = balance_weights.clone()
                    loss_combined = (
                        balance_weights_cloned[0] * losses_batch["forecast_accuracy"]
                        + balance_weights_cloned[1] * losses_batch["forecast_stability"]
                    )
                    ...
                    for i in range(0, len(losses_batch_tensor)):
                        ...
                        task_gradient = torch.autograd.grad(
                            balance_weights[i] * losses_batch[my_temp_loss],
                        ...
                        gw.append(
                            torch.linalg.norm(task_gradient)
                        )  # take L2 norm for each task
                    ...
                    self.optim.step()
                    balance_weights = (
                        balance_weights / balance_weights.sum() * T
                    ).detach()
                    ...
                    if self.shifts > 0:
                        ...
                        loss_combined = (
                            self.mylambda * losses_batch["forecast_stability"]
                ...
                avg_tloss_forecast_accuracy_epoch += (
                    losses_batch["forecast_accuracy"] / num_batches
                )
                avg_tloss_forecast_stability_epoch += (
                    losses_batch["forecast_stability"] / num_batches
                )
            ...
            if self.validation:  # validation_full and validation_earlystop
                ...
                for forigin in range(self.forigins):
                    ...
                    if forigin < self.forigins - 1:
                        eval_data_subset = np.array(
                            [
                                x[: (-18 + forigin + self.forecast_length)]
                                for x in ts_eval_pad
                            ],
                            dtype=object,
                        )
                    else:
                        eval_data_subset = np.array(
                            [x for x in ts_eval_pad], dtype=object
                        )
                    x_arr, target_arr = self.make_batch(
                        eval_data_subset, shuffle_origin=False
                    )

                    losses_evaluation = self.evaluate(
                        x_arr, target_arr, epoch, need_grad=False, early_stop=False
                    )
                    ...
                    if self.shifts > 0:
                        ...
                        loss_combined = (
                            self.mylambda * losses_evaluation["forecast_stability"]
                        ) + (
                    ...
                    avg_eloss_forecast_accuracy_epoch += (
                        losses_evaluation["forecast_accuracy"] / self.forigins
                    )
                    avg_eloss_forecast_stability_epoch += (
                        losses_evaluation["forecast_stability"] / self.forigins
                    )

                elapsed_time = time() - start_time

                print(
                    "Epoch {}/{} \t tloss_combined={:.4f} \t eloss_combined={:.4f} \t time={:.2f}s \t lambda={:.2f}".format(
                        epoch,
                        self.configNBeats["epochs"],
                ...
                print(
                    "Epoch {}/{} \t tloss_combined={:.4f} \t time={:.2f}s".format(
                        epoch,
                        self.configNBeats["epochs"],
                        avg_tloss_combined_epoch,
                        elapsed_time,
        ...
        wandb.log(
            {
                "tloss_comb": avg_tloss_combined_epoch,
        ...
        actuals = np.empty(
            shape=(len(ts_eval_pad), self.forigins, self.forecast_length)
        ...
        for forigin in range(self.forigins):
            ...
            if forigin < self.forigins - 1:
                eval_data_subset = np.array(
                    [x[: (-18 + forigin + self.forecast_length)] for x in ts_eval_pad],
                    dtype=object,
                )
            else:
        ...
        sMAPE = 200 * np.mean(
            np.abs(actuals - forecasts) / (np.abs(forecasts) + np.abs(actuals))
        )
        ...
        forecasts_helper = np.full(
            (
                actuals.shape[0],
                self.forigins,
                (self.forecast_length - 1) + self.forigins,
            ),
            np.nan,
        )
        # n_series x self.forigins x ((forecast_length - 1) + forigins)
        for forigin in range(self.forigins):
            forecasts_helper[:, forigin, forigin : (forigin + self.forecast_length)] = (
                forecasts[:, forigin, :]
            )
        MAC_mat = np.abs(np.diff(forecasts_helper, axis=1))
        # n_series x (self.forigins - 1) x ((forecast_length - 1) + forigins)
        MAC_mat_adjust = np.delete(
            MAC_mat, [0, (self.forecast_length - 1) + self.forigins - 1], 2
        )
        ...
        wandb.log({"sMAPE_m4m": sMAPE, "TotalMAC_m4m": TotalMAC})
        ...
        actuals_arr = np.column_stack(
            (
                np.repeat(np.arange(m) + 1, n),
                np.tile(np.arange(n) + 1, m),
                actuals_np.reshape(m * n, -1),
            )
        )
        actuals_df = pd.DataFrame(
            actuals_arr, columns=["item_id", "fc_origin"] + fc_colnames
        )
        ...
        forecasts_df = pd.DataFrame(
            forecasts_arr, columns=["item_id", "fc_origin"] + fc_colnames
        )
